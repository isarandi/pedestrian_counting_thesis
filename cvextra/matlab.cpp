...
void cvx::io::saveAsMatlabMat(cv::Mat const& mat,
        ...
        bpath const& folder)
{
    ...
    std::string filenamestr = (folder/(cvx::timestamp() + "_" + name + ".mat")).string();

    char const* varName = name.c_str();
    char const* filename = filenamestr.c_str();
    bool bgr2rgb=true;
